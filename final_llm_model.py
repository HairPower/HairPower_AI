...
os.environ["NGROK_AUTHTOKEN"] = "YOUR_API_KEY"
...
# OpenAI 기반 헤어스타일 추천 클래스 (UserPreferenceLLM 프롬프트 적용)
class HairstyleRecommender2:
    class State(TypedDict):
        messages: Sequence[HumanMessage]
        gender: str
        user_features: list
    ...
    def __init__(self, model_name="gpt-4o"):
        ...
        # ✅ UserPreferenceLLM 프롬프트 적용
        self.prompt_template = ChatPromptTemplate.from_messages([
            ("system",
            "✨ 당신은 헤어스타일 추천 전문가입니다. "
            "사용자의 얼굴 특징과 데이터베이스 정보를 기반으로 최적의 헤어스타일을 추천하세요. \n\n"
            "💡 [중요] 사용자의 이전 추천 스타일과 현재 요청이 조화를 이루는지 평가한 후, "
            "적절하면 이유를 설명하고, 맞지 않는 경우 더 나은 대안을 제시하세요. \n\n"
            "📌 **데이터베이스 정보:** {data} \n\n"
            "📌 **이전 추천 스타일:** {previous_recommendations}"),
            ...
            "📸 **사용자의 얼굴 특징:** {user_features} \n\n"
            "❓ **새로운 요청:** {question} \n\n"
            "✅ 새로운 요청이 적합한지 평가하고, 그렇다면 왜 좋은 선택인지 설명하세요. "
            "만약 적합하지 않다면, 더 나은 스타일을 추천하고 그 이유를 설명하세요."
            "또한 마크다운 형식으로 그리고 출력문을 한 줄로 보내주세요.")
        ])
    ...
    def recommend_hairstyle(self, user_id: str, gender: str, user_features: list, question: str = ""):
        ...
        # ✅ 기존 추천 스타일 불러오기
        previous_recommendations = [msg.content for msg in chat_history if isinstance(msg, AIMessage)]

        # ✅ OpenAI API 호출 (최종 추천)
        print(f"✅ [DEBUG] AI 호출 시작: user_id={user_id}, gender={gender}, features={user_features}, question={question}")
...
# 실행 코드 (테스트용)
if __name__ == "__main__":
    recommender = HairstyleRecommender2()
    user_id = "user123"
    gender = "male"
    user_features = ["긴 얼굴", "짧은 이마", "긴 코"]
    question = "아이비리그 컷이 하고 싶어요. 괜찮을까요?"
...
chatbot2 = HairstyleRecommender2()
app.add_middleware(
    CORSMiddleware,
    allow_origins=['*'],
...
def first_question(user_id: str = Path(..., title="User ID")):
    ...
    if isinstance(ai_response, str):
        try:
            ...
            if isinstance(parsed_response, dict) and "content" in parsed_response and "text" in parsed_response["content"]:
                ai_response_text = parsed_response["content"]["text"]  # 핵심 내용만 추출
            else:
                ai_response_text = json.dumps(parsed_response, ensure_ascii=False)  # JSON 형식이지만 content가 없을 경우 변환
        except json.JSONDecodeError:
            ai_response_text = ai_response  # JSON 변환 실패 시 그대로 사용
    else:
        ai_response_text = json.dumps(ai_response, ensure_ascii=False) if isinstance(ai_response, dict) else ai_response
    ...
    return {
        ...
        "content": {
            "type": "text/markdown",
            "text": ai_response_text  # ✅ 핵심 내용만 반환
        }
    }
...
async def chat(data: UploadChatRequest):
    ...
    # ✅ AI에게 추천 요청
    ai_response = chatbot2.recommend_hairstyle(user_id, gender, user_features, data.message)
    ...
    if isinstance(ai_response, str):
        try:
            ...
            if isinstance(parsed_response, dict) and "content" in parsed_response:
                ai_response_text = parsed_response["content"]["text"]
            else:
                ai_response_text = json.dumps(parsed_response, ensure_ascii=False)  # JSON 형식이지만 content가 없을 경우 변환
        except json.JSONDecodeError:
            ai_response_text = ai_response  # JSON 변환 실패 시 그대로 사용
